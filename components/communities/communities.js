<<<<<<< Updated upstream:components/communities/communities.js
...
IBlog.Communities = (() => {
  ...
  let _initialized = false;
  let _lastQuery   = '';
  let _communities = []; // DB communities cache
  ...
  async function _fetchCommunities() {
    try {
      ...
      // Keep IBlog.COMMUNITIES in sync for chat.js compatibility
  ...
  async function _fetchJoined() {
    try {
      ...
      const data = await res.json();
      // Store joined ids as a Set for quick lookup
      IBlog.state.joinedCommunityIds = new Set(data.map(c => String(c.id)));
  ...
  ════════════════════════════════════════════════════════ */
  function _injectShell() {
    if (document.getElementById('comm-grid')) return;
    // Shell already exists in index.html, just make sure grid is there
  }
  ...
  /* ════════════════════════════════════════════════════════
     COMMUNITY CARDS — rendered from DB data
  ════════════════════════════════════════════════════════ */
  function _buildCards(q) {
    const grid = document.getElementById('comm-grid');
    if (!grid) return;
      ...
      .map(c => {
        ...
        return `
          <div class="comm-card" id="comm-card-${c.id}">
            ...
            ${tags.length
              ? `<div class="comm-tags">${tags.map(t => `<span class="comm-tag">#${t}</span>`).join('')}</div>`
              : ''}

            <div class="comm-card-foot">
              <div class="comm-card-actions">
                ${isJoined
                  ? `<button class="comm-btn comm-btn-enter" onclick="IBlog.Communities.openChat(${c.id})">
                       Open Chat
                     </button>
                     <button class="comm-btn comm-btn-leave" onclick="IBlog.Communities.leave(${c.id}, this)">
                       Leave
                     </button>`
                  : `<button class="comm-btn comm-btn-join" id="join-btn-${c.id}" onclick="IBlog.Communities.join(${c.id}, this)">
                       Join Community
                     </button>`}
              </div>
              ${c.creatorName ? `<span class="comm-card-creator">by <strong>${c.creatorName}</strong></span>` : ''}
            </div>
          </div>`;
      });
  ...
  async function join(communityId, btnEl) {
    ...
    try {
      ...
      if (data.success || data.alreadyMember) {
        ...
        // Update IBlog.COMMUNITIES too
        const ibComm = window.IBlog.COMMUNITIES?.find(c => c.id == communityId);
        if (ibComm && !data.alreadyMember) ibComm.memberCount = comm?.memberCount;
  ...
  function openChat(communityId) {
    ...
    // Find index in IBlog.COMMUNITIES by id
...
>>>>>>> Stashed changes:backend/view/components/communities/communities.js
})();