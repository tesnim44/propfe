...
IBlog.Analytics = (() => {
  ...
  async function _loadKPIs() {
    try {
      ...
      const events = IBlogTracker?.getEvents?.() || [];
      const views = events.filter(e => e.eventType === 'view_article').length;
      const likes = events.filter(e => e.eventType === 'like_article').length;
      const comments = events.filter(e => e.eventType === 'comment_article').length;
      const articles = events.filter(e => e.eventType === 'publish_article').length;
      _counter('an-articles', articles);
      _counter('an-views', views);
      _counter('an-likes', likes);
      _counter('an-comments', comments);
    }
  }
  ...
  async function _loadChart() {
    ...
    // Fallback: derive from tracked interactions
    if (!weeks) {
      const events = IBlogTracker?.getEvents?.() || [];
      ...
      for (let i = 0; i < 12; i++) {
        ...
        const key = `${d.getFullYear()}-W${String(Math.ceil((((d - new Date(d.getFullYear(),0,1)) / 86400000) + new Date(d.getFullYear(),0,1).getDay()+1)/7)).padStart(2,'0')}`;
        weekMap.set(key, { label: key, views: 0, likes: 0 });
      }
      events.forEach(evt => {
        const d = new Date(evt.at || Date.now());
        const key = `${d.getFullYear()}-W${String(Math.ceil((((d - new Date(d.getFullYear(),0,1)) / 86400000) + new Date(d.getFullYear(),0,1).getDay()+1)/7)).padStart(2,'0')}`;
        const row = weekMap.get(key);
        if (!row) return;
        if (evt.eventType === 'view_article') row.views += 1;
        if (evt.eventType === 'like_article') row.likes += 1;
      });
