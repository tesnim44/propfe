...
IBlog.Activity = (() => {
  ...
  function _renderFallbackHeatmap() {
    ...
    const days = Array.from({ length: 365 }, (_, i) => {
      ...
      const r = Math.random();
      const count = i > 330 ? (r > .4 ? Math.floor(r * 8) + 1 : 0)
                 : i > 270  ? (r > .6 ? Math.floor(r * 5) + 1 : 0)
                 :             (r > .75 ? Math.floor(r * 3) + 1 : 0);
      return { date: date.toISOString().split('T')[0], count };
    });

    _renderHeatmap(days);

    // Fallback stats from JS state
    const user = IBlog.state?.currentUser;
    const arts = (IBlog.state?.articles || []).filter(a => a.author === user?.name);
    _setEl('act-posts',    arts.length);
    _setEl('act-comments', arts.reduce((s, a) => s + (a.comments?.length || 0), 0));
    _setEl('act-saves',    IBlog.state?.savedArticles?.length || 0);
    _setEl('act-streak',   Math.floor(Math.random() * 30 + 1));
  }
