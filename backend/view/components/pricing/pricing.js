(function () {
   ...
   */
  const AUTH_API = 'iblog/backend/view/components/auth/api-auth.php';
  const MAIL_API = 'iblog/backend/view/components/auth/api-mail.php';
        ...
        <div class="pricing-modal">
          ...
          <div class="pricing-modal-form" id="checkout-form-wrap">
            ...
            <div class="co-step" id="step-payment" style="display:none">
              ...
              <div class="pricing-form-group"><label for="co-paymethod">Payment method</label><select id="co-paymethod"><option value="">Select a method</option><option value="card">Bank card (Visa / Mastercard)</option><option value="d17">D17 / Flouci</option><option value="konnect">Konnect</option><option value="sobflous">Sobflous</option><option value="ccp">Banque Postale (CCP)</option></select><span class="field-error" id="err-co-paymethod"></span></div>
  ...
  function onPayMethodChange() {
    ...
    if (method === 'd17')     $('wallet-info-text').textContent = 'After confirming, you will receive a payment request on your D17 / Flouci app.';
  ...
  function buildSummary() {
    ...
    const amount = isAnnual ? activePlan.price.annual : activePlan.price.monthly;
    const method = val('co-paymethod');
    const methodLabels = { card: 'Bank card (Visa / Mastercard)', d17: 'D17 / Flouci', konnect: 'Konnect', sobflous: 'Sobflous', ccp: 'Banque Postale (CCP)' };
