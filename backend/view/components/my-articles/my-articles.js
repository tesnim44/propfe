...
// Gere l'affichage, la suppression et l'edition des articles
// Namespace : window.MyArticles + integration IBlog.Auth.toast
// ============================================================
...
// ============================================================
  ...
  function _readArticles() {
    ...
    // Premier acces sans donnees injectees
    return [];
  }
  ...
  function load() {
    ...
    if (!container) return;
    if (window.IBlog?.Views?.buildMyArticles) {
  ...
  function deleteArticle(id) {
    ...
    if (Array.isArray(window.IBlog?.state?.articles)) {
      ...
      window.IBlog.state.articles = window.IBlog.state.articles.filter(a => a.id !== normalizedId && String(a.id) !== String(id));
