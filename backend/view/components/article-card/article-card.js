...
IBlog.ArticleCard = {
  ...
  render(article, index = 0, options = {}) {
    ...
    const userHasPremium = IBlog.state.currentUser?.plan === 'premium';
    const insightLabel = article.likes > 500
      ...
      ? 'Widely shared'
      : 'Editor pick';
    const impactValue = article.likes > 0
      ? `${Math.min(99, Math.max(58, Math.round(article.likes / 8)))}`
      : '64';
      ...
      <div class="card-body">
        ...
        <div class="card-story-grid">
          ...
          <div class="card-impact-box">
            <span class="card-impact-kicker">Heat</span>
            <strong>${impactValue}%</strong>
            <small>${(article.tags||[])[0] || (article.category||article.cat||'General')}</small>
          </div>
        </div>
        <div class="card-meta">
        ...
        <button class="pod-toggle-btn locked" onclick="IBlog.ArticleCard.togglePodcast(${article.id},this)">
          <span class="pod-icon">${this.SVG.mic}</span>
          <span id="pod-label-${article.id}">Podcast Preview</span>
          <span class="pod-premium-pill">Premium</span>
        </button>`}
  ...
  togglePodcast(id, btn) {
    if (IBlog.state.currentUser?.plan !== 'premium') {
      IBlog.utils.toast('Podcast listening is a premium feature.');
      if (typeof showPremium === 'function') showPremium();
      return;
    }
