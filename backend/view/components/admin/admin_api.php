...
/**
 * admin_api.php
 * Location : iblog/backend/view/components/admin/admin_api.php
 * API JSON interne — utilisée par admin.js via fetch().
 *
 * Toutes les fonctions du controller retournent des tableaux associatifs.
 * On accède donc toujours avec $row['colonne'] et jamais $row->colonne.
 */
...
if (empty($_SESSION['adminLoggedIn']) || $_SESSION['adminLoggedIn'] !== true) {
    ...
    echo json_encode(['error' => 'Forbidden — vous n\'êtes pas connecté en tant qu\'admin']);
    exit();
}

// ── Chargement de la base de données ─────────────────────────────────────────
// __DIR__ = .../iblog/backend/view/components/admin  → 3 niveaux up = backend
